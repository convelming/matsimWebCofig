...
### 二、首页（[home/index.vue](./src/views/home/index.vue)）
...
- 数据依赖 [home.js](./src/api/home.js) API（新闻/附件/PDF 水印/热点链接）

### 三、新闻/公告详情页（[home/News/](./src/views/home/News)）
- **路由**：`/news/detail`（type=0） & `/notice/detail`（type=1，同一组件不同 meta）
- [NewsDetail.vue](./src/views/home/News/NewsDetail.vue)：从 query.id 取 `newsDetail(id)`，含附件（多文件下载/批量下载 `newsAnnexBatchDownload`），调用 `pdfWatermark` 对 PDF 自动打水印
- [AddNews.vue](./src/views/home/News/AddNews.vue)：`@vueup/vue-quill` 富文本编辑器 + 附件上传 + `newsAdd / newsUpdate / newsDelete`

---

### 四、数据上传（[upload/index.vue](./src/views/upload/index.vue)）
- **路由**：`/upload`（MapLayout 子路由）
- 默认展示左侧折叠菜单（MDialog 包裹，宽度 300px），共 **7 大类**（4/5/6 为预留空菜单，研发中），点击已实现的子菜单项切换到对应功能面板：

| 一级菜单 | 二级项目 | 面板组件 | 功能 |
|------|------|------|------|
| 1. 人工数车 | 1-1 路段流量录入 | [LinkFlow/index.vue](./src/views/upload/LinkFlow/index.vue)（含 `AELinkFlow` + `LinkDetail` + `CopyFlow`） | 路段流量 CRUD；复制流量到反向/全 Way；AELinkFlow 快速编辑 |
| | 1-2 交叉口流量录入 | [IntersectionFlow/index.vue](./src/views/upload/IntersectionFlow/index.vue)（含 `AddIntersection` / `CrossroadsDetail` / `CrossroadsStatsEdit` / `DrawLine` / `InputCrossroads` + ManuallyEntering + `VideoInputCrossroads` + `AEIntersectionFlow`） | 交叉口新增/删除/编辑、画出入口方向线、16 向流量表、AI 视频车辆计数、封面帧、视频/轨迹/Excel 导出 |
| 2. 航拍视频 | 2-1 仅视频 | [UploadVideo/index.vue](./src/views/upload/UploadVideo/index.vue) | 视频批量上传（交评/项目/路口分类） |
| | 2-2 路段流量识别 | ——（ElMessage 研发中） | |
| | 2-3 交叉口流量识别 | ——（ElMessage 研发中） | |
| 3. 拍照图片 | 3-1 仅图片 | [UploadImage/index.vue](./src/views/upload/UploadImage/index.vue)（含 [Upload.vue](./src/views/upload/UploadImage/Upload.vue) / [ImageList.vue](./src/views/upload/UploadImage/ImageList.vue) + [ImagePreview.vue](./src/views/upload/UploadImage/ImagePreview.vue) + [Sreach.vue](./src/views/upload/UploadImage/Sreach.vue)） | ZIP 批量上传、按文件夹/项目管理、地图锚点预览、搜索查询 `Sreach` |
| 4. GIS 文件 | ——（空 children） | ——（ElMessage 研发中） | 预留 |
| 5. CAD 文件 | ——（空 children） | ——（ElMessage 研发中） | 预留 |
| 6. 栅格文件 | ——（空 children） | ——（ElMessage 研发中） | 预留 |
| 7. 交评道路运行现状出图 | 7-1 开始出图 | [CTProject/index.vue](./src/views/upload/CTProject/index.vue)（含 `AddDialog` + `ListDialog` + `DrawingModelDialog` + `ModeDialog`） | 新建/载入出图项目、绘制模式、出图模型、饱和度样本服务水平设置 |

---

### 五、数据下载（[download/index.vue](./src/views/download/index.vue)）
- **路由**：`/download`（MapLayout 子路由）
- 双功能：**数据树下载** + **空间查询下载**

#### 5.1 数据树（[download/Tree/](./src/views/download/Tree)）
- [download/Tree/index.js](./src/views/download/Tree/index.js)：自定义渲染 Tree 组件，枚举 `TreeItemEnum`（10 种类型）
- 每种类型对应 [download/Tree/components/](./src/views/download/Tree/components)：
  | 组件名 | 用途 |
  |------|------|
...
- [mixins.js](./src/views/download/Tree/mixins.js)（老项目遗留封装，兼容复用）

#### 5.2 空间查询（[download/SpatialQuery/](./src/views/download/SpatialQuery)）
- [SpatialQuery/index.vue](./src/views/download/SpatialQuery/index.vue)（含 [mixins.js](./src/views/download/SpatialQuery/mixins.js) + [components/SelectArea.vue](./src/views/download/SpatialQuery/components/SelectArea.vue) + [components/RouteInfo.vue](./src/views/download/SpatialQuery/components/RouteInfo.vue)）
...
### 六、问题反馈（[feedback/index.vue](./src/views/feedback/index.vue)）
- **路由**：`/feedback`（MapLayout 子路由）
- 外链跳转至独立反馈系统：`http://192.168.60.231:23105/vue/feedback.html#/`

---

## 通用业务图层工具（[utils/MapLayer/](./src/utils/MapLayer)）

road_traffic_survey2 相比老版本，对业务图层做了统一模块化升级：

| 图层文件 | 用途 |
|------|------|
| [GeoJSONLayer.js](./src/utils/MapLayer/GeoJSONLayer.js) + [GeoJSONLayer.worker.js](./src/utils/MapLayer/GeoJSONLayer.worker.js) | GeoJSON 大规模要素 Worker 渲染（支持 Point/MultiPoint/LineString/Polygon + 颜色映射） |
| [GeoJSONLayer/](./src/utils/MapLayer/GeoJSONLayer) ❗新目录版 | [layer.js](./src/utils/MapLayer/GeoJSONLayer/layer.js) / [point.js](./src/utils/MapLayer/GeoJSONLayer/point.js) / [line.js](./src/utils/MapLayer/GeoJSONLayer/line.js) / [polygon.js](./src/utils/MapLayer/GeoJSONLayer/polygon.js) / [data.js](./src/utils/MapLayer/GeoJSONLayer/data.js) / [utils.js](./src/utils/MapLayer/GeoJSONLayer/utils.js) / [worker.worker.js](./src/utils/MapLayer/GeoJSONLayer/worker.worker.js) + [point.svg](./src/utils/MapLayer/GeoJSONLayer/point.svg)：面向组件的重构版 |
| [SelectLayer/](./src/utils/MapLayer/SelectLayer) ❗新目录版 | `CircleSelectLayer / RectangleSelectLayer / PolygonSelectLayer（含 PathLayer + [utils.js](./src/utils/MapLayer/SelectLayer/utils.js) + [index.js](./src/utils/MapLayer/SelectLayer/index.js)）`：3 种空间查询交互层（下载页使用） |
| [LinkLayer.js](./src/utils/MapLayer/LinkLayer.js) | 单路段交互选择 |
| [LinksLayer.js](./src/utils/MapLayer/LinksLayer.js) | 批量路段 + 双向偏移/着色 |
| [LinkStatsLayer.js](./src/utils/MapLayer/LinkStatsLayer.js) | 路段流量与颜色条 |
| [GuangZhouLayer.js](./src/utils/MapLayer/GuangZhouLayer.js) | 广州专题边界（与老版本共享） |
| [NetworkLayer.js](./src/utils/MapLayer/NetworkLayer.js) | MATSim 3D 网络 |
| [PolygonsLayer.js](./src/utils/MapLayer/PolygonsLayer.js) | 批量多边形层 |
| [IntersectionListLayer.js](./src/utils/MapLayer/IntersectionListLayer.js) | 交叉口 Marker 列表 |
| [ImageListLayer.js](./src/utils/MapLayer/ImageListLayer.js) | 图片 Marker 树 |
| [DrawLineLayer.js](./src/utils/MapLayer/DrawLineLayer.js) | 画线交互 |
| [FrameSelectLayer.js](./src/utils/MapLayer/FrameSelectLayer.js) | 矩形框选 |
| [PointSelectLayer.js](./src/utils/MapLayer/PointSelectLayer.js) / [PolygonSelectLayer.js](./src/utils/MapLayer/PolygonSelectLayer.js) | 点选 / 多边形圈选 |
| [ColorBar2D.js](./src/utils/MapLayer/ColorBar2D.js) + [ColorBar2DUtil.js](./src/utils/MapLayer/ColorBar2DUtil.js) | 颜色图例组件 |
...
| 组件 | 说明 |
|------|------|
| [MButton.vue](./src/components/MButton.vue) | 万能按钮：支持 `type=router`（路由）、`type=a`（新窗口链接）、`record` 埋点（研发中）、`activeClass`、带 SVG icon + label |
| [Dialog.vue](./src/components/Dialog.vue)（全局注册为 **MDialog**） | [main.js](./src/main.js):`app.component('MDialog', Dialog)`，可拖拽：最小化、隐藏关闭、标题/副标题、x/y 定位 |
| [Pagination.vue](./src/components/Pagination.vue)（全局注册为 **MPagination**） | Element Plus 分页封装（pageSize/size-change/current-change） |
| [RouteSelect.vue](./src/components/RouteSelect.vue) | 路段下拉搜索 + 关键字模糊匹配 + 图标 |
| [Sector.vue](./src/components/Sector.vue) | 扇区统计组件（与老版本共享） |
| [UploadVideo.vue](./src/components/UploadVideo.vue)（全局注册为 **MUploadVideo**） | 视频上传组件（交评/项目/路口分类） |
| QuillEditor（全局注册，来自 @vueup/vue-quill） | 新闻/公告富文本编辑器（[main.js](./src/main.js) 注册 snow 主题 CSS） |

---

## 路由配置

- **入口**：[src/router/index.js](./src/router/index.js)，Hash 模式（`createWebHashHistory`）
...
## API 接口（[src/api/](./src/api)）

### 1. 核心业务（[api/index.js](./src/api/index.js)）
...
### 2. 首页（[api/home.js](./src/api/home.js)）
...
### 3. 下载门户（[api/download.js](./src/api/download.js)）
| 函数 | 方法 | 路径 | 说明 |
|------|------|------|------|
...
## 核心 3D 地图引擎（[src/mymap/](./src/mymap)）

与 road_traffic_survey / ocean_web 同架构，是共享的统一版本：

| 模块 | 文件 | 说明 |
|------|------|------|
| **入口** | [index.js](./src/mymap/index.js) | 导出 `MyMap / Layer / MapLayer / OutlineLayer`（4 个核心） |
| **核心** | [main/MyMap.js](./src/mymap/main/MyMap.js) + [main/Layer.js](./src/mymap/main/Layer.js) + [main/EventListener.js](./src/mymap/main/EventListener.js) | Three.js 场景、鼠标手势、enableRotate、zoom、图层事件总线 |
| **内置图层** | [layers/MapLayer.js](./src/mymap/layers/MapLayer.js)（多风格瓦片切换 MapStyle） + [layers/OutlineLayer.js](./src/mymap/layers/OutlineLayer.js)（3D 建筑白膜） | |
| **后期合成** | [composer/BloomComposer.js](./src/mymap/composer/BloomComposer.js) | Bloom 辉光 |
| **材质** | [material/Line2DMaterial.js](./src/mymap/material/Line2DMaterial.js) | 屏幕空间 2D 宽线 Shader |
| **工具** | [utils/BaiduTileUtils.js](./src/mymap/utils/BaiduTileUtils.js) / [utils/OSMTileUtils.js](./src/mymap/utils/OSMTileUtils.js) / [utils/LngLatUtils.js](./src/mymap/utils/LngLatUtils.js) / [utils/ColorBar2D.js](./src/mymap/utils/ColorBar2D.js) / [utils/ModelPool.js](./src/mymap/utils/ModelPool.js) / [utils/CSS2DRenderer.js](./src/mymap/utils/CSS2DRenderer.js) | 坐标/瓦片/图例/GLTF 对象池/DOM 标注 |
| | [utils/lines/Line2.js](./src/mymap/utils/lines/Line2.js) / [utils/lines/LineSegments2.js](./src/mymap/utils/lines/LineSegments2.js) + Geometry + Material | Fat Line 宽线系列 |
| **天空盒** | [skybox/](./src/mymap/skybox)（[front.jpg](./src/mymap/skybox/front.jpg),[back.jpg](./src/mymap/skybox/back.jpg),[left.jpg](./src/mymap/skybox/left.jpg),[right.jpg](./src/mymap/skybox/right.jpg),[up.jpg](./src/mymap/skybox/up.jpg),[down.jpg](./src/mymap/skybox/down.jpg)） | 6 面 + [readme.txt](./src/mymap/skybox/readme.txt) |
...
### 交通 SVG 图标集（[assets/icon_traffic/](./src/assets/icon_traffic)）
...
### UI 切图（[assets/images/Home/](./src/assets/images/Home) + [MapLayout/](./src/assets/images/MapLayout)）
Home：数据库/工具库/平台库/热门功能/新闻/标签/搜索/上传/实时报导/日期/区域/下载/宣传/图标等 SVG/PNG
MapLayout：4 个菜单（upload/download/home/feedback）+ 4 个工具栏按钮图标

### 其他
- [Circle.svg](./src/assets/images/Circle.svg) / [Rectangle.svg](./src/assets/images/Rectangle.svg) / [Polygon.svg](./src/assets/images/Polygon.svg)：空间查询选图按钮
- [bg_line.png](./src/assets/images/bg_line.png) / [bg_wave02@2x.png](./src/assets/images/bg_wave02@2x.png) / [地图@2x.png](./src/assets/images/地图@2x.png) / [图钉.png](./src/assets/images/图钉.png) / [xlsx.svg](./src/assets/images/xlsx.svg) / [附件.svg](./src/assets/images/附件.svg) / [defult_xinpian@2x.png](./src/assets/images/defult_xinpian@2x.png) / [close.svg](./src/assets/images/close.svg) / [address.png](./src/assets/images/address.png) / [sreach.svg](./src/assets/images/sreach.svg)

### 字体 / 样式
- [assets/style/base.scss](./src/assets/style/base.scss) / [func.scss](./src/assets/style/func.scss) / [main.scss](./src/assets/style/main.scss)（[func.scss](./src/assets/style/func.scss) 全局通过 `additionalData` 注入所有 `<style lang="scss">`，免 import）
...
| 业务图层 | home.back/layer + home2/layer（分散） | 统一为 [utils/MapLayer/](./src/utils/MapLayer)（含 SelectLayer 新架构与 GeoJSON 分目录版） |
