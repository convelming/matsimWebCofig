...
export class MyMap extends EventListener {
  ...
  get cameraRotation() {
    try {
      ...
      const [px, py, pz] = new THREE.Vector3().subVectors(
  ...
  initRenderer() {
    ...
    this.rootDoc.appendChild(this.css2dRenderer.domElement)
  }
  ...
  initComposer() {
    ...
    this.smaaPass = new SMAAPass(
      this.rootDoc.clientWidth * pixelRatio,
      this.rootDoc.clientHeight * pixelRatio,
    )
      ...
      if (!this.MOUSE_MOVE_TIMEOUT) {
        this.MOUSE_MOVE_TIMEOUT = setTimeout(() => {
          ...
          const layer = this.layers.find(
            (v) => v.usePick && v.pickLayerColor.getHex() == pickLayerColor,
          )
  ...
  handleGPUPickLayer(x, y) {
    ...
    const pickLayerColor =
      (pixelBufferLayer[0] << 16) | (pixelBufferLayer[1] << 8) | pixelBufferLayer[2]
  ...
  getTileRangeByZoom(zoom) {
    ...
    const [row, col] = [
      Math.floor(((EARTH_RADIUS + mapCenterX) * Math.pow(2, zoom)) / (EARTH_RADIUS * 2)),
      Math.floor(((EARTH_RADIUS - mapCenterY) * Math.pow(2, zoom)) / (EARTH_RADIUS * 2)),
    ]
  ...
  WindowXYToCanvasXY(x, y) {
    ...
    const y3 = 0
    const px =
      ((origin.x - direction.x) / (origin.y - direction.y)) * (y3 - direction.y) + direction.x
    const pz =
      ((origin.z - direction.z) / (origin.y - direction.y)) * (y3 - direction.y) + direction.z
