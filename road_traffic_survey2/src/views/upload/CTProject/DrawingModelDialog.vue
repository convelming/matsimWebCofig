...
<template>
        ...
        <el-collapse-item title="基础信息" name="0">
          ...
          >
            ...
            <el-form-item label="项目时间：" prop="projectTime">{{
              form.projectTime.slice(0, 7)
            }}</el-form-item>
            <el-form-item label="项目范围：" :prop="{ 1: 'xyarr', 2: 'file' }[form.xyarrType]">
              <el-radio-group
                v-model="form.xyarrType"
                size="mini"
                @change="handleStopPolygonSelect(true)"
              >
              ...
              <template v-if="form.xyarrType == 1">
                <div style="margin-top: 10px">
                  <el-button
                    ...
                    type="primary"
                    size="mini"
                    @click="handlePlayPolygonSelect('changeDetail')"
                    >开始圈定</el-button
                  >
                  <template v-if="selectState != POLYGON_SELECT_STATE_KEY.NOT_STARTED">
                    <el-button
                      type="primary"
                      size="mini"
                      @click="handleReplayPolygonSelect('changeDetail')"
                      >重新圈定</el-button
                    >
                    <el-button
                      type="primary"
                      size="mini"
                      @click="handleStopPolygonSelect('changeDetail')"
                      >结束圈定</el-button
                    >
                  </template>
                </div>
              ...
              <template v-if="form.xyarrType == 2">
                <div style="margin-top: 10px">
                  <el-button type="primary" size="mini" @click="handleSelectFile"
                    >上传shp</el-button
                  >
                </div>
        ...
        <el-collapse-item title="框选批量赋值：饱和度&服务水平" name="1">
          <div>
            <el-button
              ...
              type="primary"
              size="mini"
              @click="handlePlayPolygonSelect('changeLinksData')"
              >开始圈定</el-button
            >
            <template v-if="selectState != POLYGON_SELECT_STATE_KEY.NOT_STARTED">
              <el-button
                type="primary"
                size="mini"
                @click="handleReplayPolygonSelect('changeLinksData')"
                >重新圈定</el-button
              >
              <el-button
                type="primary"
                size="mini"
                @click="handleStopPolygonSelect('changeLinksData')"
                >结束圈定</el-button
              >
            </template>
          </div>
        ...
        <el-collapse-item title="样式调整" name="3">
          <el-form :model="styleForm" ref="styleForm" label-width="120px" size="mini">
            <el-form-item>
              ...
              <el-slider v-model="styleForm.linkWidth" range show-stops :min="0" :max="20">
              </el-slider>
            </el-form-item>
            <el-form-item>
              <template slot="label">
            ...
            <el-form-item>
              ...
              <div style="display: flex; flex-direction: column; gap: 10px">
                <div
                  v-for="item in styleForm.colorList"
  ...
  <MDialog
    ...
    width="300px"
    hideMinimize
    :visible="showChangeLinksDialog"
    @close="handleCloseChangeLinksDialog"
  >
    <div class="cpt_dm_bodyer">
      <el-form
        :model="changeLinksForm"
        ref="changeLinksForm"
        :rules="changeLinksRules"
        label-width="100px"
        :inline="false"
        size="mini"
      >
        <el-form-item label="饱和度：" prop="saturation">
          <el-input-number
            v-model="changeLinksForm.saturation"
            :min="0"
            :step="0.01"
            :controls="true"
          />
        ...
        <el-form-item label="服务水平：" prop="service">
          <el-select v-model="changeLinksForm.service">
            <el-option v-for="item in 'ABCDEF'" :key="item" :label="item" :value="item">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
...
import { PolygonsLayer } from '@/utils/MapLayer/PolygonsLayer'
import { LinksLayer } from '@/utils/MapLayer/LinksLayer'
import {
  projectDetail,
  projectList,
  projectUpdate,
