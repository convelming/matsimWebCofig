    ...
    // 创建点数据数组
    {
      ...
      for (const { propertiesKey, geometry } of pointList) {
        for (const [x, y] of geometry) {
          ...
          geomList[propertiesKey].pointArray.push(x, y, propertiesKey);
        }
      }
    ...
    // 创建线数据数组
    {
      ...
      for (const { propertiesKey, geometry } of lineList) {
        for (const line of geometry) {
          ...
          geomList[propertiesKey].lineArray.push(l.length + 1);
          geomList[propertiesKey].lineArray.push(propertiesKey);
          geomList[propertiesKey].lineArray.push(...l);
        }
      }
    ...
    // 创建多边形数据数组
    {
      ...
      for (const { propertiesKey, geometry } of polygonList) {
        for (const polygon of geometry) {
          ...
          geomList[propertiesKey].polygonArray.push(shapeList.length + 1);
          geomList[propertiesKey].polygonArray.push(propertiesKey);
          geomList[propertiesKey].polygonArray.push(...shapeList);
        }
      }
