...
export class NetworkLayer extends Layer {
    ...
    try {
      ...
      for (let i = 0, l = data.length; i < l; i++) {
        ...
        list[list.length] = {
          ...
          path: geomJson.coordinates
            .map((v) => [Math.round(v[0] - center[0]), Math.round(v[1] - center[1])])
            .flat(),
          data: data[i],
        }
      }
  ...
  getGeometry() {
    ...
    for (const data of this.data) {
      ...
      const attrPosition = new THREE.BufferAttribute(new Float32Array(length * 4 * 3), 3)
      const attrStartPosition = new THREE.BufferAttribute(new Float32Array(length * 4 * 2), 2)
      const attrEndPosition = new THREE.BufferAttribute(new Float32Array(length * 4 * 2), 2)
      const attrSide = new THREE.BufferAttribute(new Float32Array(length * 4), 1)
      const attrIndex = new THREE.BufferAttribute(new Uint16Array(length * 2 * 3), 1)
      const attrUv = new THREE.BufferAttribute(new Float32Array(length * 4 * 2), 2)
      const attrPickColor = new THREE.BufferAttribute(new Float32Array(length * 4 * 3), 3)
      const attrColor = new THREE.BufferAttribute(new Float32Array(length * 4 * 3), 3)
      ...
      for (let index = 0; index < data.path.length; index++) {
        ...
        const nextLink = index == data.path.length - 1 ? link : data.path[index + 1]
