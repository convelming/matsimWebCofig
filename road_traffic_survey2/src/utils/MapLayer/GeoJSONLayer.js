...
export const ICON_LIST = Object.values(
  import.meta.glob('@/assets/icon_traffic/*.{png,jpg,svg}', { eager: true }),
...
const defaultParams = {
  ...
  pointIcon: new URL('@/assets/icon_traffic/point.svg?url', import.meta.url).href,
  pointValue: '',
  pointColorBar: [],
  pointOpacity: 1,
  // ******************** 线 ******************** //
...
export class GeoJSONLayer extends Layer {
  ...
  async updatePoint() {
    ...
    for (let i = 0, l = this.pointArray.length; i < l; i += maxPoint * 3) {
      ...
      const geometry = new GeoJSONPointListGeometry(
        pointArray,
  ...
  async updateLine() {
    ...
    for (
      let index = 0, l = this.lineArray.length, dataSize = this.lineArray[0], num = 0;
      index < l;
      index += 1 + dataSize, dataSize = this.lineArray[index]
    ) {
  ...
  async updatePolygon() {
    ...
    const addMesh = (list) => {
      const geometry = new GeoJSONPolygonListGeometry(
        list,
        this.propertiesLabels,
        this.polygonValue,
        this.polygonValue3D,
      ...
      const borderGeometry = new GeoJSONPolygonBorderListGeometry(
        list,
        this.propertiesLabels,
        this.polygonValue,
        this.polygonValue3D,
    ...
    for (
      let index = 0, l = this.polygonArray.length, num = 0, dataSize = this.polygonArray[0];
      index < l;
      index += 1 + dataSize, dataSize = this.polygonArray[index]
    ) {
...
export class GeoJSONPolygonListGeometry extends THREE.BufferGeometry {
  constructor(polygonList = [], propertiesLabels = {}, valueKey = '', value3DKey = '') {
    ...
    function getShape(array) {
      ...
      let shape = null
      for (
        let j = 0, l = shapeArray.length, size = shapeArray[0];
        j < l;
        j += 1 + size, size = shapeArray[j]
      ) {
