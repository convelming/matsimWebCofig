...
export function getColor(types, colors, def_color) {
  try {
    if (!types) throw new Error('types is empty')
    const typeList = String(types).split(',')
    const colorList = typeList.map((type) => new THREE.Color(colors[type]).getHex())
    const sum = colorList.reduce((acc, num) => acc + num, 0)
    return new THREE.Color(sum / colorList.length)
  } catch (error) {
    return new THREE.Color(def_color)
  }
}
...
export class LinkLayer extends Layer {
  ...
  constructor(options) {
    ...
    this.selectColor = options.selectColor || '#ffffff'
    ...
    for (const data of this.data) {
      for (let index = 0; index < data.length; index++) {
        ...
        if (link.id == this.selectId) {
          ...
          const color = getColor(link.statsType, this.colors, this.colors[0])
