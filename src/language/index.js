class Language {
  ...
  constructor() {
    ...
    this.page_language =
      sessionStorage.getItem("language") || this.page_default_language;
  }
  ...
  internationalize(
    key,
    dict = this.base_lanuage_dict,
    page_language = this.page_language
  ) {
    try {
      return (
        dict[key][page_language] || dict[key][this.page_default_language] || ""
      );
    } catch (error) {
      return "";
    }
  ...
  install(Vue, options = {}) {
    ...
    _self.page_default_language =
      options.page_default_language || _self.page_default_language;
    _self.base_lanuage_dict =
      options.base_lanuage_dict || _self.base_lanuage_dict;
    _self.$event = new Vue();
    ...
    Vue.mixin({
      ...
      computed: {
        $l() {
          return (key) =>
            _self.internationalize(key, this.language, this.page_language);
        },
      },
      beforeCreate() {
