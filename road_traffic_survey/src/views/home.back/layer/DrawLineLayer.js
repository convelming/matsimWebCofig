...
export class DrawLineLayer extends Layer {
  ...
  constructor(opt) {
    ...
    if (this.url) {
      ...
      geometry.setAttribute("position", new THREE.Float32BufferAttribute([0, 0, 0, 0, -this.height, 0, this.width, -this.height, 0, this.width, 0, 0], 3));
    ...
    if (opt.drawLineList) {
      this.drawLineList = opt.drawLineList.map(v => new DrawLine(v.name, [v.beginx, v.beginy], [v.endx, v.endy], v.color));
...
class DrawLine {
  ...
  updateGeometry() {
    this._geometry.setAttribute('position', new THREE.Float32BufferAttribute([this.begin[0], -this.begin[1], 0.1, this.end[0], -this.end[1], 0.1], 3));
