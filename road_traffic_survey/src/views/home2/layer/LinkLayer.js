...
export class LinkLayer extends Layer {
  ...
  constructor(options) {
    ...
    this.colors = options.colors || COLOR_LIST;
  ...
  getSelectGeometry() {
    ...
    for (const data of this.data) {
      for (let index = 0; index < data.length; index++) {
        ...
        if (link.id == this.selectId) {
          ...
          const color = getColor(link.statsType, this.colors) || DEFAULT_COLOR;
    ...
    geometry.setAttribute("startPosition", new THREE.BufferAttribute(new Float32Array(attrStartPosition.flat()), 2));
    geometry.setAttribute("endPosition", new THREE.BufferAttribute(new Float32Array(attrEndPosition.flat()), 2));
    geometry.setAttribute("side", new THREE.BufferAttribute(new Float32Array(attrSide.flat()), 1));
    geometry.setAttribute("uv", new THREE.BufferAttribute(new Float32Array(attrUv.flat()), 2));
    geometry.setAttribute("pickColor", new THREE.BufferAttribute(new Float32Array(attrPickColor.flat()), 3));
    geometry.setAttribute("color", new THREE.BufferAttribute(new Float32Array(attrColor.flat()), 3));
    ...
    for (const data of this.data) {
      {
        ...
        for (let index = 0; index < data.length; index++) {
          ...
          if (index == data.length - 1) {
            ...
            attrPickColor.setXYZ(index * 4 + 4, pickColor.r, pickColor.g, pickColor.b);
            attrPickColor.setXYZ(index * 4 + 5, pickColor.r, pickColor.g, pickColor.b);
