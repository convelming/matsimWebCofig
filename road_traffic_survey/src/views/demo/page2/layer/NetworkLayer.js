...
function getTileNetwork(params) {
  return fetch(`http://192.168.60.231:23105/pt/network/getTileNetwork?${new URLSearchParams(params).toString()}`, {
    headers: {
      uuid: guid(),
      Datasource: encodeURIComponent("广州/30wSurvey"),
    },
  })
...
export class NetworkTile {
  static noodMap = new THREE.TextureLoader().load(require("@/assets/image/point.svg"));
  static videoIconMap = new THREE.TextureLoader().load(require("@/assets/image/camera-fill.svg"));
