...
export class RouteFlowsLayer extends Layer {
  ...
  constructor(opt) {
    ...
    this.material2 = new THREE.MeshLambertMaterial({ color: "red" });
  ...
  on(type, data) {
    ...
    if (type == MAP_EVENT.HANDLE_MOUSE_MOVE_PICK) {
      ...
      if (data.layerId == this.id) {
        ...
        if (this.selectMesh) this.selectMesh.material = this.material;
        if (item) {
          this.selectMesh = this.meshMap.get(pickColor);
          if (this.selectMesh) mesh.material = this.material2;
  ...
  setData(stops, data, maxValue) {
    try {
      ...
      for (const value of Object.values(data)) {
        const fromStop = stops.get(value.source);
        const toStop = stops.get(value.target);
        if (fromStop && toStop && value.value > 0) {
          ...
          const item = {
            ...
            fromStop: stops.get(value.source),
            toStop: stops.get(value.target),
            value: value.value,
  ...
  update() {
    ...
    if (this.data) {
      for (const value of this.data) {
        ...
        this.meshMap.set(value.pickColorNum, mesh);
        this.scene.add(mesh);
