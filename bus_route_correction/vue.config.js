...
module.exports = defineConfig({
  // 默认打包时会为输出的 JS、CSS 等静态资源文件名添加 内容哈希（content hash）
  // filenameHashing: false,
  // 用于指定哪些 第三方依赖（node_modules 中的包）需要被 Babel 转译（transpile）
  ...
  outputDir: process.env.VUE_APP_OUTPUT_DIR || "static",
  ...
  devServer: {
    ...
    proxy: {
      "/": {
        ws: false,
        target: `http://192.168.152.22:23105`, // 本地调试
